#Remove Other
execute as @e[tag=gateway_portal] unless entity @s[distance=..2] if score @e[sort=nearest,limit=1] gateway = @s gateway run summon item ~ ~ ~ {Item:{id:"minecraft:phantom_spawn_egg",Count:1b,tag:{display:{Name:'{"text":"Gateway","italic":false}'},EntityTag:{id:"minecraft:area_effect_cloud",Tags:["gateway_point"],Radius:0.0f,Duration:10},CustomModelData:4268769}}}
...
#Remove This
summon item ~ ~ ~ {Item:{id:"minecraft:phantom_spawn_egg",Count:1b,tag:{display:{Name:'{"text":"Gateway","italic":false}'},EntityTag:{id:"minecraft:area_effect_cloud",Tags:["gateway_point"],Radius:0.0f,Duration:10},CustomModelData:4268769}}}
forceload remove ~ ~
kill @s
