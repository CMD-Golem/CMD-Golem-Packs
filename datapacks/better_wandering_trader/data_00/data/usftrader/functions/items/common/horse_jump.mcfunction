#get the health
execute store result score @s usftrader_store run data get entity @s Attributes[{Name:"horse.jumpStrength"}].Base 100
#add 2 healths
scoreboard players add @s usftrader_store 5
#if the score isn't over 30 set new health
execute unless score @s usftrader_store matches 105.. store result entity @s Attributes[{Name:"horse.jumpStrength"}].Base double 0.01 run scoreboard players get @s usftrader_store
