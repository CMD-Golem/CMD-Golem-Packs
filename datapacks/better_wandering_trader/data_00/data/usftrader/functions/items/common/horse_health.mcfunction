#get the health
execute store result score @s usftrader_store run data get entity @s Attributes[{Name:"generic.maxHealth"}].Base
#add 2 healths
scoreboard players add @s usftrader_store 2
#if the score isn't over 30 set new health
execute unless score @s usftrader_store matches 32.. store result entity @s Attributes[{Name:"generic.maxHealth"}].Base double 1 run scoreboard players get @s usftrader_store
