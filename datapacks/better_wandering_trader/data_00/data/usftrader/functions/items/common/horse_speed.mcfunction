#get the health
execute store result score @s usftrader_store run data get entity @s Attributes[{Name:"generic.movementSpeed"}].Base 1000
#add 2 healths
scoreboard players add @s usftrader_store 10
#if the score isn't over 30 set new health
execute unless score @s usftrader_store matches 340.. store result entity @s Attributes[{Name:"generic.movementSpeed"}].Base double 0.001 run scoreboard players get @s usftrader_store
